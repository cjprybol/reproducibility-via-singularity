...
Services like [GitHub](https://github.com/), [Bitbucket](https://bitbucket.org), and [GitLab](https://about.gitlab.com/) have democratized access to affordable (or free!) tools that reinforce reproducability in research, via the code repositories that these services offer. These services make it easier to backup, version control, collaborate on, and distribute code (or any text-based file for that matter). Those features make it easier for the researcher to write and maintain high-quality code. Those features also increase the chance that someone else will review the code for errors or bugs. This review can be done by some combination of reading and executing the code. Unfortunately, unless the code is run on the exact same computer, while logged in as the same user, getting the same code to run the same way can be a research project in and of itself.


# Why Singularity?
Universities and research laboratories often conduct their work on shared HPC clusters. These clusters are all set up on slightly different configurations of hardware and operating system software. Trying to recreate an environment to re-run code exactly as it was executed on another cluster, an environment where all of the C pointers align in the same way, the Java versions sync up, and where multicore libraries are configured correctly (and the software versions match!) is beyond the abilities, time availabilities, and account priviliges of most researchers. Virtual Machines offer one approach to mitigate this problem. Virtual machines can provide each user (or user group) with a clean and isolated environment, where those users can install any software and run any code they desire, without fear of affecting other users, and without worries about conflicting software dependencies with other users. However, virtual machines have problems that have prevented them from being widely adopted as a standard in research. Singularity is a runtime container that has been specifically developed for the needs of HPC cluster users and staff. By installing of the software required to execute an analysis inside of a singularity container, you can isolate the computing environment needed to run the code from the computing resources needed to run it. The singularity container simply acts as an interpreter that receives instructions, and translates those instructions to work on whatever cluster configuration you have access to.
